#!/usr/bin/env python3
"""Merge the finalized flow matching_RG paper summary into the site catalog."""
...
CATEGORY_TAGS: dict[str, tuple[str, ...]] = {
    "Few-step and one-step generation": ("Machine Learning", "Flow Matching", "Few-step Generation"),
    "Flow matching and diffusion models": ("Machine Learning", "Flow Matching", "Diffusion Models"),
    "Flow matching practice and benchmarking": ("Machine Learning", "Flow Matching", "Benchmarking"),
    "Generative-model theory and path measures": ("Machine Learning", "Flow Matching", "Path Integrals"),
    "Renormalization group and generative models": ("Machine Learning", "Flow Matching", "Renormalization Group", "Statistical Physics"),
    "Symmetry and conservation laws": ("Machine Learning", "Symmetry"),
    "Unified generative modeling": ("Machine Learning", "Generative Models", "Flow Matching"),
}
...
def main() -> None:
    ...
    for paper in papers:
        ...
        item = {
            "id": hashlib.sha256(f"flow-matching-rg:{stable_key}".encode("utf-8")).hexdigest()[:16],
            "date": args.date,
            "title": title,
            "url": url,
    ...
    audit = {
        ...
        "unique_catalog_matches": len({item["id"] for item in catalog if "Flow Matching RG" in item["tags"]}),
