...
本站把日常阅读、专题筛选和外部学者收藏中的论文线索整理为长期可检索的个人研究索引。它用于发现研究方向、保存阅读入口和回到原始文献，不是论文质量排行榜，也不替代同行评审。

- 可按标题、发表来源、主题、策展来源和阅读优先级搜索与筛选；
- 可按发表日期、收录日期、主题或标题排序；
- 星标保存在当前浏览器，并支持 JSON 导入、导出；
- Atom/RSS 提供新增论文订阅；
- 已制作全文 Paper Card 的论文可进入详情页继续阅读和讨论。

来自其他学者公开收藏的论文会在 `curation_sources` 中保留策展来源、原始链接、筛选等级和日期。2026-08-30 导入的批次来自 [刘子鸣的 Paper Collection](https://metacircleai.github.io/ziming-paper-collection/collection.html)，先限定为质量评估前 50%，再保留 S、A+、A 阅读优先级。该标记说明发现与策展来源，不表示原作者或刘子鸣认可本站的二次筛选。
...
- `curation_sources`：可选的策展来源数组，记录公开收藏名称、链接、本站筛选等级与日期；它与论文的发表来源分开保存。
